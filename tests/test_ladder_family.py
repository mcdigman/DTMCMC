...
def test_entropy_ladder_reproduces_frozen_fixture(
    ...
    ladder = EntropyTemperatureLadder(
        n_chain, Ts_in, vars_in, n_cold=n_cold, T_cold=T_cold, n_inf_final=n_inf_final, correct_last=correct_last
    )
    assert_allclose(ladder.Ts, fixture[key], rtol=1.0e-10, atol=0.0)


def _original_heat_capacity_integrated(
    logL_vars_use: np.ndarray, betas_use: np.ndarray, correct_last: bool
) -> np.ndarray:
...
def test_generalized_machinery_bit_exact_vs_original(gold_inputs, correct_last) -> None:
    ...
    generalized = get_spacing_integrated(vars_use.copy(), betas_use.copy(), correct_last, p=1.0, q=1.0)
...
def test_gaussian_null_case_ladders_coincide() -> None:
    ...
    """
    n_par = 4
    Ts_in = np.geomspace(1.0, 64.0, 33)
    vars_in = n_par * Ts_in**2 / 2.0
    n_chain = 9

    entropy_ladder = EntropyTemperatureLadder(n_chain, Ts_in, vars_in, n_cold=1, T_cold=1.0, n_inf_final=0)
    length_ladder = LengthTemperatureLadder(n_chain, Ts_in, vars_in, n_cold=1, T_cold=1.0, n_inf_final=0)
    geometric_ladder = GeometricTemperatureLadder(n_chain, 1, 1.0, 1.0, 64.0, n_inf_final=0)
...
def test_acceptance_predictor_vs_brute_force(beta1, delta_beta) -> None:
    ...
    mean1, mean2 = -n_par / (2.0 * beta1), -n_par / (2.0 * beta2)
    var1, var2 = n_par / (2.0 * beta1**2), n_par / (2.0 * beta2**2)
    ...
    brute_force = float(np.where(ratio >= 0.0, 1.0, np.exp(np.minimum(ratio, 0.0))).mean())
...
def test_acceptance_ladder_equal_acceptance(cake1_inputs) -> None:
    ...
    n_chain = 12
    ladder = AcceptanceTemperatureLadder(n_chain, Ts_in, means_in, vars_in, n_cold=1, T_cold=1.0, n_inf_final=1)

    assert ladder.Ts.size == n_chain
    assert ladder.n_cold == 1
    assert np.sum(~np.isfinite(ladder.Ts)) == 1
    assert 0.0 < ladder.achieved_acceptance < 1.0
    ...
    acceptances = np.array(
        [
            predicted_swap_acceptance(
                betas[itrt],
                betas[itrt + 1],
                interp_scalar(mean_interp, betas[itrt]),
                interp_scalar(mean_interp, betas[itrt + 1]),
                interp_scalar(var_interp, betas[itrt]),
                interp_scalar(var_interp, betas[itrt + 1]),
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            'kind': 'acceptance_file',
            'n_chain': 8,
            'n_cold': 1,
            'Ts_file': 'data/Ts_cake1.npy',
            'means_file': 'data/means_cake1.npy',
            'vars_file': 'data/vars_cake1.npy',
        },
    ],
)
...
def _gaussian_ladder_stats(
    n_par: int, Ts_max: float = 64.0, n_points: int = 17, include_inf: bool = False
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Analytic Gaussian-target ladder inputs, optionally with a prior rung."""
    Ts_in = np.geomspace(1.0, Ts_max, n_points)
    betas = 1.0 / Ts_in
    means = -n_par / (2.0 * betas)
    variances = n_par / (2.0 * betas**2)
    if include_inf:
        # a beta = 0 (infinite temperature) entry with finite prior stats
        Ts_in = np.append(Ts_in, np.inf)
        means = np.append(means, -200.0)
        variances = np.append(variances, 1.0e4)
...
def test_acceptance_ladder_prunes_unrequested_inf() -> None:
    ...
    ladder = AcceptanceTemperatureLadder(9, Ts_in, means, variances, n_cold=1, T_cold=1.0, n_inf_final=0)
...
def test_acceptance_inf_edge_outside_contract() -> None:
    ...
    n_chain = 9
    ladder = AcceptanceTemperatureLadder(n_chain, Ts_in, means, variances, n_cold=1, T_cold=1.0, n_inf_final=1)
    assert np.sum(~np.isfinite(ladder.Ts)) == 1
...
def test_sort_mode_zero_forwarded_by_ladder_classes() -> None:
    ...
    for ladder_class, lower_Ts in (
        (
            EntropyTemperatureLadder(9, Ts_in, variances, n_cold=1, T_cold=4.0, n_inf_final=0, sort_mode=0),
            entropy_spaced_betas(9, 1, Ts_in, variances, n_inf_final=0, T_cold=4.0, sort_mode=0)[1],
        ),
        (
            LengthTemperatureLadder(9, Ts_in, variances, n_cold=1, T_cold=4.0, n_inf_final=0, sort_mode=0),
            entropy_spaced_betas(9, 1, Ts_in, variances, n_inf_final=0, T_cold=4.0, sort_mode=0, p=0.5, q=0.0)[1],
        ),
        (
            AcceptanceTemperatureLadder(9, Ts_in, means, variances, n_cold=1, T_cold=4.0, n_inf_final=0, sort_mode=0),
            acceptance_spaced_betas(9, 1, Ts_in, means, variances, n_inf_final=0, T_cold=4.0, sort_mode=0)[1],
        ),
    ):
        # T_cold=4 sits mid-range, so the unsorted (cold-first) order is
        # genuinely different from sorted — the class must preserve it
        assert not np.all(np.diff(lower_Ts) >= 0.0)
...
def test_predicted_acceptance_stays_in_probability_range() -> None:
    ...
    """
    repro = predicted_swap_acceptance(0.0, 1.0e-6, 0.0, 0.0, 1.0e30, 1.0e30)
    assert 0.0 <= repro <= 1.0
    assert repro == pytest.approx(0.5, abs=0.01)

    for beta1 in (0.0, 1.0e-6, 0.5, 1.0):
        for beta2 in (0.0, 1.0e-6, 0.5, 1.0):
            for mean_delta in (0.0, 1.0e6, -1.0e6):
                for var in (1.0e-6, 1.0, 1.0e30):
                    a = predicted_swap_acceptance(beta1, beta2, 0.0, mean_delta, var, var)
                    assert 0.0 <= a <= 1.0, (beta1, beta2, mean_delta, var, a)
...
def test_cake_reference_normalizes_amps() -> None:
    ...
    """
    n_par = 5
    widths, exponents = (2.0, 0.5), (4.0, 2.0)
    ...
    assert scaled == pytest.approx(normalized, rel=1.0e-14)
...
def test_tunable_cake_family() -> None:
    ...
    assert_allclose(
        (draws**2).sum(axis=1).mean(), cake_moment_r2(n_par, amps=amps, widths=widths, exponents=exponents), rtol=0.05
    )
