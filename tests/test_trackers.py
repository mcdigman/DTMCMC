...
def test_esd_accumulation_direct() -> None:
    ...
    mcmc_decision_helper(1, samples, logLs, betas, accept_record, esd_record, 0, np.array([3.0, 4.0]), 1.0e6, 0.0, 0)
    assert esd_record[0, 0, 0] == pytest.approx(25.0)
    assert esd_record[1, 0, 0] == pytest.approx(25.0)
    assert accept_record[0, 0, 0] == 1

    # forced reject: -inf likelihood; proposed sum grows, accepted does not
    samples[1] = 0.0
    mcmc_decision_helper(1, samples, logLs, betas, accept_record, esd_record, 0, np.array([0.0, 2.0]), -np.inf, 0.0, 0)
    assert esd_record[0, 0, 0] == pytest.approx(29.0)
    assert esd_record[1, 0, 0] == pytest.approx(25.0)
...
def test_esd_exchange_accumulation_direct() -> None:
    ...
    samples[1] = [[0.0, 0.0], [3.0, 4.0]]
    logLs = np.zeros((3, n_chain))
    logLs[1] = [0.0, 5.0]
    betas = np.array([1.0, 0.1])
    ...
    em.do_ptmcmc_exchange(
        1, samples, logLs, n_chain, betas, exchange_tracker, esd_exchange, chain_track, em.SEQUENTIAL_TARGETS, False
    )

    assert_array_equal(chain_track[2], [1, 0])
    assert_array_equal(samples[2], [[3.0, 4.0], [0.0, 0.0]])
    assert esd_exchange[0] == pytest.approx(25.0)
    assert esd_exchange[1] == pytest.approx(25.0)

    # null targeting proposes nothing: no displacement accumulates
    esd_null = np.zeros(n_chain)
    em.do_ptmcmc_exchange(
        1, samples, logLs, n_chain, betas, exchange_tracker, esd_null, chain_track, em.NULL_TARGETS, False
    )
    assert_array_equal(esd_null, [0.0, 0.0])
...
def test_trackers_on_real_run() -> None:
    ...
    assert np.all(tracker.esd_record >= 0.0)
    assert np.all(tracker.esd_record[1] <= tracker.esd_record[0] + 1.0e-12)
    # jump types that were proposed have positive proposed-ESD
    proposed_counts = tracker.accept_record.sum(axis=0)
    assert np.all((tracker.esd_record[0] > 0.0) == (proposed_counts > 0))
    ...
    assert np.all(tracker.esd_exchange >= 0.0)
    assert_array_equal(tracker.esd_exchange > 0.0, accepted_exchanges > 0)
