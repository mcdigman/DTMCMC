...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog='dashboard',
        description='DTMCMC telemetry dashboard (live monitoring or post-hoc analysis of run artifacts)',
    )
    parser.add_argument('artifact', type=Path, help='run artifact .h5 file, or a directory to browse')
    parser.add_argument(
        '--host', default='127.0.0.1', help='bind address (default localhost; use an ssh tunnel for remote viewing)'
    )
    parser.add_argument('--port', type=int, default=8050)
    parser.add_argument('--poll', type=float, default=5.0, metavar='SECONDS', help='artifact poll interval (default 5)')
    ...
    parser.add_argument(
        ...
        default=600.0,
        metavar='SECONDS',
        help='flag a non-finalized artifact as stale after this flush age',
    )
    parser.add_argument(
        '--no-store', action='store_true', help='skip loading the sample store (lighter polling; posterior tab empty)'
    )
