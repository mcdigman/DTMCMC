...
def fig_flow_history(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            z=flow.f_per_block.T,
            zmin=0.0,
            zmax=1.0,
            colorscale=sequential_colorscale(theme),
            colorbar={
                'title': {'text': 'f', 'font': {'color': theme.ink_secondary}},
...
def fig_round_trips(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=trips.itrns_cold,
            y=trips.cumulative_cold,
            name='arrived cold (hot→cold)',
            mode='lines',
            line={'color': theme.categorical[0], 'width': 2.0},
    ...
    fig.add_trace(
        go.Scatter(
            x=trips.itrns_hot,
            y=trips.cumulative_hot,
            name='arrived hot (cold→hot)',
            mode='lines',
    ...
    for segment_itrn in trips.segment_itrns:
        fig.add_vline(x=int(segment_itrn), line={'color': theme.gridline, 'width': 1.0})
    return fig
...
def fig_walker_arrivals(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(
            x=trips.walker_ids,
            y=trips.cold_arrivals_per_walker,
            name='cold arrivals',
...
def fig_logl_block_history(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    for slot in range(snapshot.n_chain):
        fig.add_trace(
            go.Scatter(
                x=history.blocks,
                y=history.values[:, slot],
                name=f'slot {slot} ({hover_tags[slot]})',
                mode='lines',
...
def fig_de_eigvals(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig.update_layout(
        title={
            'text': f'checkpoint at iteration {int(spectrum.itrns[-1])}',
...
def fig_de_effective_rank(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    spectrum = diag.de_spectrum_summary(snapshot)
    fig = go.Figure(
        layout=base_layout(theme, x_title='temperature T', y_title='effective rank (participation ratio)', log_x=True)
    )
    if spectrum.itrns.size == 0:
        return annotate_empty(fig, theme, 'no DE-spectrum checkpoints recorded')
    ...
    for idx in range(n_checkpoints):
        ...
        fig.add_trace(
            go.Scatter(
                x=Ts_arr[finite],
                y=spectrum.eff_rank[idx][finite],
                name=f'itrn {int(spectrum.itrns[idx])}',
                mode='lines',
                line={
                    'color': theme.highlight if is_latest else colors[min(idx, len(colors) - 1)],
                    'width': 2.5 if is_latest else 1.4,
                },
    ...
    if n_par > 0:
        fig.add_hline(
            y=n_par,
            line={'color': theme.ink_muted, 'dash': 'dot', 'width': 2.0},
            annotation_text=f'full rank {n_par}',
