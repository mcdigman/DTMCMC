...
def fig_corner(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    for col in range(n_dims):
        fig.update_xaxes(
            title={'text': labels[col], 'font': {'color': theme.ink_secondary, 'size': 11}},
            showticklabels=True,
            row=n_dims,
            col=col + 1,
        )
    for row in range(1, n_dims):
        fig.update_yaxes(
            title={'text': labels[row], 'font': {'color': theme.ink_secondary, 'size': 11}},
            showticklabels=True,
            row=row + 1,
            col=1,
        )
    ...
    fig.update_layout(
        title={
            'text': f'{diag.store_column_label(snapshot, opts.chain)}, {n_points} stored points',
            'font': {'size': 12, 'color': theme.ink_secondary},
        }
    )
    return fig
...
def _acf_traces(results: list[diag.AcfResult], snapshot: RunSnapshot, theme: Theme) -> list[go.Scatter]:
    ...
    for itrc, result in enumerate(results):
        ...
        label = (
            result.label
            if not np.isfinite(result.tau_int)
            else f'{result.label} (τ≈{result.tau_int * snapshot.store_thin:.0f} itrn)'
        )
        traces.append(
            go.Scatter(
                x=result.lags * snapshot.store_thin,
                y=result.rho,
                name=label,
                mode='lines',
...
def fig_logl_acf(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig = go.Figure(
        data=_acf_traces(results, snapshot, theme),
        layout=base_layout(theme, x_title='lag (iterations)', y_title='logL autocorrelation'),
    )
    fig.add_hline(y=0.0, line={'color': theme.baseline, 'width': 1.0})
    if not results:
...
def fig_parameter_acf(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig = go.Figure(
        data=_acf_traces(results, snapshot, theme),
        layout=base_layout(theme, x_title='lag (iterations)', y_title='parameter autocorrelation'),
    )
    fig.add_hline(y=0.0, line={'color': theme.baseline, 'width': 1.0})
    if not results:
...
def fig_logl_trace(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    curves = [
        diag.Curve(
            f'logL {diag.store_column_label(snapshot, chain)}',
            rows * snapshot.store_thin,
            snapshot.logLs[rows, chain],
            'applied',
        )
    ...
    fig = go.Figure(
        data=curve_traces(curves, theme), layout=base_layout(theme, x_title='iteration', y_title='stored logL')
    )
    if not curves:
        annotate_empty(fig, theme, 'no stored logL for the selected chains')
    return fig
