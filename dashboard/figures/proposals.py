...
def _rate_table_traces(table: diag.RateTable, theme: Theme, *, value_name: str) -> list[go.Scatter]:
    ...
    for itrj, label in enumerate(table.labels):
        ...
        traces.append(go.Scatter(
            x=Ts_plot, y=table.values[finite, itrj], name=label, mode='lines+markers',
            line={'color': color, 'width': 2.}, marker={'size': 7, 'color': color},
...
def fig_exchange_vs_temperature(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=Ts_plot, y=rates.nn_rate[finite], name='nearest neighbor', mode='lines+markers',
    ...
    if not np.allclose(rates.all_rate[finite], rates.nn_rate[finite], equal_nan=True):
        fig.add_trace(go.Scatter(
            x=Ts_plot, y=rates.all_rate[finite], name='all exchanges', mode='lines+markers',
            line={'color': theme.categorical[1], 'width': 2.}, marker={'size': 7},
    ...
    if np.isfinite(rates.overall_nn_rate):
        fig.add_hline(y=rates.overall_nn_rate, line={'color': theme.ink_muted, 'dash': 'dot', 'width': 2.}, annotation_text=f'overall {rates.overall_nn_rate:.3f}', annotation_font={'color': theme.ink_muted})
    fig.update_yaxes(range=[0., 1.])
    return fig
...
def fig_exchange_history(snapshot: RunSnapshot, opts: ViewOptions, theme: Theme) -> go.Figure:
    ...
    fig = go.Figure(layout=base_layout(theme, x_title='archive window (ends at hovered iteration)', y_title='temperature slot (cold → hot)'))
