...
def tuned_geometric_t_max(n_chain: int) -> float:
    ...
    keep = Ts_in >= 1.
    ladder = EntropyTemperatureLadder(n_chain, Ts_in[keep], vars_in[keep], n_cold=1, T_cold=1., n_inf_final=1)
...
def arm_ladders(n_chain: int) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        'length': {'kind': 'length_file', 'n_chain': n_chain, 'n_cold': 1, 'Ts_file': 'data/Ts_cake_gold.npy', 'vars_file': 'data/vars_cake_gold.npy'},
...
def main() -> int:
    ...
    specs = [
        ...
        for arm_name, ladder in ladders.items() for seed in SEEDS
    ]
    ...
    for arm_name in ladders:
        ...
        print(f'{arm_name:>18}: rt_rate {metrics[arm_name]["rt_rate"].mean():8.3f} +- {metrics[arm_name]["rt_rate"].std():6.3f}   '
    ...
    results: dict[str, Any] = {
        ...
        'arms': {arm: {key: values.tolist() for key, values in arm_metrics.items()} for arm, arm_metrics in metrics.items()},
        'power': {},
    }
    ...
    for arm_name in ladders:
        ...
        for metric_name in ('rt_rate', 'n_eff_per_eval'):
            ...
            print(f'entropy vs {arm_name:>18} [{metric_name:>14}]: effect {np.median(diffs):+.3e}, paired sd {diffs.std(ddof=1):.3e}, seeds needed {needed:.1f}')
