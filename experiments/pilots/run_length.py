...
def main() -> int:
    ...
    half = n_iterations // 2
    rate_first = round_trip_rate(
        events[(events[:, 1] > n_iterations // 4) & (events[:, 1] <= half)], 16, half - n_iterations // 4
    )
    rate_second = round_trip_rate(events[events[:, 1] > half], 16, n_iterations - half)
    long_metrics = load_run_metrics(artifact_path)
    print(
        f'long run: rate {long_metrics["rt_rate"]:.3f}; first half (post 25% burn) {rate_first:.3f} vs second half {rate_second:.3f}'
    )
    ...
    for arm_name, arm_metrics in arms['arms'].items():
        ...
        if mean_rts > 0.0:
            ...
            required = TARGET_ROUND_TRIPS / mean_rts * (n_steps_pilot / 2.0)
        else:
            required = float('inf')
