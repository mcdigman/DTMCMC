...
def get_cumulants(l_means):
    ...
    l_cum5 = (l5_means - 5 * l4_means * l1_means - 10 * l3_means * l2_means +
            20 * l3_means * l1_means**2 + 30 * l2_means**2 * l1_means - 60 * l2_means * l1_means**3 + 24 * l1_means**5)
    l_cum6 = (l6_means - 6 * l5_means * l1_means - 15 * l4_means * l2_means + 30 * l4_means * l1_means**2 - 10 * l3_means**2 +
              120 * l3_means * l2_means * l1_means - 120 * l3_means * l1_means**3 + 30 * l2_means**3 - 270 * l2_means**2 * l1_means**2 +
              360 * l2_means * l1_means**4 - 120 * l1_means**6)
...
def get_averaged_adjacents(mcc, length, cut=0):
    ...
    l_p11_avg = np.mean(l_p11_means[cut:].reshape(((l_p11_means.shape[0] - cut) // length, length, l_p11_means.shape[1])), axis=1)
    l_p21_avg = np.mean(l_p21_means[cut:].reshape(((l_p11_means.shape[0] - cut) // length, length, l_p21_means.shape[1])), axis=1)
    l_p12_avg = np.mean(l_p12_means[cut:].reshape(((l_p11_means.shape[0] - cut) // length, length, l_p12_means.shape[1])), axis=1)
...
def get_corr_quantities(l_means, l_adjacents):
    ...
    l_coskew21 = (l_p21 - 2 * x_mean * l_p11 + x_mean**2 * y_mean - x2_mean * y_mean + 2 * x_mean**2 * y_mean - x_mean**2 * y_mean)
    l_coskew12 = (l_p12 - 2 * y_mean * l_p11 + y_mean**2 * x_mean - y2_mean * x_mean + 2 * y_mean**2 * x_mean - y_mean**2 * x_mean)

    return [l_cov, l_coskew12, l_coskew21], [l_cov / (x_std * y_std), l_coskew21 / (x_std**2 * y_std), l_coskew12 / (x_std * y_std**2)]
