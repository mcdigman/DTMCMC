...
def get_default_proposal_manager(T_ladder, like_obj, strategy_params, starting_samples,
    ...
    if fisher_manager_loc is None:
        fisher_manager_loc = fm.FisherJumpManager(T_ladder, like_obj, starting_samples, strategy_params)

    if de_manager_loc is None:
        de_manager_loc = dm.DEJumpManager(T_ladder, like_obj, strategy_params)

    if auxilliary_manager_loc is None:
        auxilliary_manager_loc = am.AuxilliaryJumpManager(T_ladder, like_obj, strategy_params)

    if prior_manager_loc is None:
        prior_manager_loc = pm.PriorManager(T_ladder, like_obj, strategy_params)
