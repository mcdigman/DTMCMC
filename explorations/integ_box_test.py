...
rs = np.linspace(0.0, np.sqrt(n_dim * cutoff**2), n_r)
...
def areaFunc(rs):
    ...
    r_high = rs[rs > cutoff]
    res[rs > cutoff] = 2**n_dim * cutoff * (r_high**2 - cutoff**2) ** ((n_dim - 1) / 2) + np.pi ** (
        -n_dim / 2
    ) * r_high**n_dim * (4 * np.pi**n_dim - 4**n_dim * np.pi * np.arccos(cutoff / r_high) ** (n_dim - 1)) / (
        4 * gamma(1 + n_dim / 2)
    )
    r_low = rs[rs <= cutoff]
...
def rCutFunc(rs):
    return 2**n_dim * cutoff * (n_dim - 1) * rs * (rs**2 - cutoff**2) ** ((n_dim - 3) / 2) + np.pi ** (
        n_dim / 2
    ) / gamma(1 + n_dim / 2) * (
        -(
            4 ** (n_dim - 1)
            ...
            * np.pi ** (1 - n_dim)
            * rs
            * (rs * np.arccos(cutoff / rs)) ** (n_dim - 2)
        )
        / np.sqrt(rs**2 - cutoff**2)
        + n_dim * rs ** (n_dim - 1) * (1 - (np.pi / 4) ** (1 - n_dim) * np.arccos(cutoff / rs) ** (n_dim - 1))
    )


integrand[rs > cutoff] = rCutFunc(rs[rs > cutoff]) / rCutFunc(cutoff + 1.0e-13) * cutoff ** (n_dim - 1)

res = 2 * np.pi / (np.pi / 2.0) * cumulative_trapezoid(integrand, rs, initial=0)
...
bins_rec, bin_locs = np.histogram(rs_sim, 100000, range=(0.0, np.sqrt(n_dim) * cutoff))
...
for itrb in range(max_itrb):
    rs_sim = np.sqrt(np.sum(np.random.uniform(-cutoff, cutoff, (n_sim, n_dim)) ** 2, axis=1))
    bins_loc, _ = np.histogram(rs_sim, 100000, range=(0.0, np.sqrt(n_dim) * cutoff))
    bins_rec += bins_loc
    tl = perf_counter()
...
plt.plot(rs, 1 / cutoff**n_dim * np.pi / 2.0 * integrand)
plt.show()
