...
if __name__ == '__main__':
    ...
    params_true = np.array(
        [like_obj.xs_grid[max_index[0]], like_obj.ys_grid[max_index[1]]]
    )  # make sure the conventions on the parameters match
    ...
    density1 = np.histogram2d(mcc.samples_store[10000:, 0, 0], mcc.samples_store[10000:, 0, 1], bins=[x_bins, y_bins])[
        0
    ]
    ...
    for itrt in range(1, n_chain - 1):
        ...
        density_t = np.histogram2d(
            mcc.samples_store[10000:, itrt, 0], mcc.samples_store[10000:, itrt, 1], bins=[x_bins, y_bins]
        )[0]
        density_t /= density_t.sum()
        density_translate = density_t ** (1.0 / T_ladder.betas[itrt])
