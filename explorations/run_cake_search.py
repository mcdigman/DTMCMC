...
if __name__ == '__main__':
    ...
    T_max = 1.0e3  # maximum temperature for geometric part of temperature ladder
    ...
    T_ladder = th.entropy_ladder_fromfile(
        n_chain,
        n_cold,
        'data/Ts_cake_gold.npy',
        'data/vars_cake_gold.npy',
        n_inf_final=1,
        T_cold=1.0,
        correct_last=False,
    )
    ...
    if do_corner_plot:
        ...
        samples_format, params_true_format = like_obj.format_samples_output(
            mcc.samples_store[n_burnin:, argT_1, :].copy(), params_true
        )
    ...
    if do_corr_plots:
        ...
        for itrt in range(max(0, argT_1 - 5), min(n_chain, argT_1 + 5)):
            ...
            autocorr_logL_lim = np.hstack(
                [
                    autocorr_logL[n_use - 1 : n_use],
                    autocorr_logL[n_use : 2 * n_use - 2 : 2] + autocorr_logL[n_use + 1 : 2 * n_use - 1 : 2],
                ]
            )
    ...
    for itrt in range(max(0, argT_1 - 10), min(n_chain, argT_1)):
        ...
        b2 = 2 * n_use
        plt.plot(
            crosscorr_logL[b1 : b2 - 5 : 4]
            + crosscorr_logL[b1 + 1 : b2 - 4 : 4]
            + crosscorr_logL[b1 + 2 : b2 - 3 : 4]
            + crosscorr_logL[b1 + 3 : b2 - 2 : 4]
        )
    ...
    if do_heat_plot_gold:
        ...
        plt.loglog(
            T_ladder.Ts[argTs],
            (
                np.mean(np.array(mcc.logL2_means[block_burnin:]), axis=0)
                - np.mean(np.array(mcc.logL_means[block_burnin:]), axis=0) ** 2
            )[argTs],
        )
        plt.show()
    ...
    cumulants = np.array(
        moment_helpers.get_cumulants(
            moment_helpers.get_averaged_means(mcc, len(mcc.logL_means) - block_burnin, cut=block_burnin)
        )
    )[:, 0]
    ...
    arg_cut_r = np.argmax(autocorr_rs_lim / autocorr_rs_lim[0] < 0.0)
    ...
    res_even, binsx, binsy = np.histogram2d(
        rs_stack[0 : rs_stack.size - 1 : 2], np.diff(rs_stack)[::2], 100, range=[[0, 6.3], [-6.3, 6.3]]
    )
    res_odd, binsx, binsy = np.histogram2d(
        rs_stack[1 : rs_stack.size - 1 : 2], np.diff(rs_stack)[1::2], 100, range=[[0, 6.3], [-6.3, 6.3]]
    )
    ...
    for itrb in range(10):
        ...
        res_shuffle_loc, binsx, binsy = np.histogram2d(
            rs_shuffle[0 : rs_shuffle.size - 1 :], np.diff(rs_shuffle), 100, range=[[0, 6.3], [-6.3, 6.3]]
        )
        res_shuffle += res_shuffle_loc
        empty_log.append(np.sum(res_shuffle == 0.0))
    ...
    for itrt in range(1, n_chain - 1):
        accept_exchange_nn_right[itrt] = accept_exchange[itrt, itrt + 1]
        accept_exchange_nn_left[itrt] = accept_exchange[itrt, itrt - 1]
        accept_exchange_nn[itrt] = (a_ex_yes[itrt, itrt + 1] + a_ex_yes[itrt, itrt - 1]) / (
            a_ex_yes[itrt, itrt + 1] + a_ex_no[itrt, itrt + 1] + a_ex_yes[itrt, itrt - 1] + a_ex_no[itrt, itrt - 1]
        )
    ...
    print(
        np.min(diff_ext_log[cycle_burn_index:]),
        np.max(diff_ext_log[cycle_burn_index:]),
        np.median(diff_ext_log[cycle_burn_index:]),
        np.mean(diff_ext_log[cycle_burn_index:]),
        np.std(diff_ext_log[cycle_burn_index:]),
    )
    print(
        np.min(cycle_zero_log[cycle_burn_index:]),
        np.max(cycle_zero_log[cycle_burn_index:]),
        np.median(cycle_zero_log[cycle_burn_index:]),
        np.mean(cycle_zero_log[cycle_burn_index:]),
        np.std(cycle_zero_log[cycle_burn_index:]),
    )
    print(
        np.max(cycle_min_log[cycle_burn_index:]),
        np.median(cycle_min_log[cycle_burn_index:]),
        np.mean(cycle_min_log[cycle_burn_index:]),
        np.std(cycle_min_log[cycle_burn_index:]),
    )
    ...
    plt.scatter(
        unique_rs[recurr_lengths_mask], np.log10(recurr_lengths[recurr_lengths_mask]), s=0.1, alpha=1.0, color='black'
    )
    plt.show()

    plt.scatter(unique_rs[recurr_lengths_mask], count_rs[recurr_lengths_mask], s=0.1, alpha=1.0, color='black')
    plt.show()
    ...
    rs_got = rs_got[rs_got < 10.0]
    counts, bins, _ = plt.hist(rs_got, 1000, density=True)
    bins_match = np.unique(np.hstack([np.linspace(0.0, 1.0, 100), bins, np.array([10.0])]))
    bin_likes = np.zeros(len(bins_match))
    for itrb in range(bin_likes.size):
        bin_likes[itrb] = like_obj.get_loglike(np.array([bins_match[itrb], 0.0, 0.0, 0.0, 0.0]))
    ...
    cov_means = np.cov(
        (moment_helpers.get_averaged_means(mcc, (len(mcc.logL_means) - block_burnin) // 10, cut=block_burnin))[0].T
    )
    ...
    plt.plot(
        moment_helpers.get_corr_quantities(
            moment_helpers.get_averaged_means(mcc, len(mcc.logL_means) - block_burnin, cut=block_burnin),
            moment_helpers.get_averaged_adjacents(mcc, len(mcc.logL_means) - block_burnin, cut=block_burnin),
        )[1][0][0]
    )
    plt.show()
    ...
    betas_new = np.unique(
        np.hstack(
            [
                mcc.betas,
                betas_old,
                np.linspace(0.5, 2.0, 10000),
                10 ** np.linspace(np.log10(max_beta), -10.0, 10000),
                np.linspace(max_beta, 0.0, 10000),
            ]
        )
    )[::-1]
    ...
    estim_left, estim_right, estim_center = integral_heat_estimator.cumulant_heat_cap_interp(
        cumulants, mcc.betas, betas_new
    )
    estim_left1, estim_right1, estim_center1 = integral_heat_estimator.cumulant_heat_cap_interp(
        cumulants[:2], mcc.betas, betas_new
    )
    estim_left2, estim_right2, estim_center2 = integral_heat_estimator.cumulant_heat_cap_interp(
        cumulants[:3], mcc.betas, betas_new
    )
    estim_left3, estim_right3, estim_center3 = integral_heat_estimator.cumulant_heat_cap_interp(
        cumulants[:4], mcc.betas, betas_new
    )
    estim_left4, estim_right4, estim_center4 = integral_heat_estimator.cumulant_heat_cap_interp(
        cumulants[:5], mcc.betas, betas_new
    )
    ...
    cycle_count = np.array(
        [
            np.sum((mcc.tracker_manager.cycle_archive[itrb][2] + mcc.tracker_manager.cycle_archive[itrb][3]) / 2)
            for itrb in range(len(mcc.tracker_manager.itrn_archive))
        ]
    )
    new_cycles = np.hstack([cycle_count[0], np.diff(cycle_count)])
    new_iterations = np.hstack(
        [mcc.tracker_manager.itrn_archive[0], np.diff(np.array(mcc.tracker_manager.itrn_archive))]
    )
    ...
    plt.plot(
        np.array(mcc.tracker_manager.itrn_archive[cycle_burn_index:]),
        np.full(np.array(mcc.tracker_manager.itrn_archive[cycle_burn_index:]).size, mean_rate),
        'k--',
    )
    plt.ylim(0.0, 0.001)
    plt.show()
    ...
    acceptance_log = np.zeros(
        (len(mcc.tracker_manager.exchange_archive), n_chain, mcc.tracker_manager.accept_archive[0].shape[2])
    )
