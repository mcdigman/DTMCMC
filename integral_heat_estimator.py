...
def cumulant_integrand(cumulants, betas):
    ...
    for itrt in range(n_chain - 1):
        for itrc in range(len(cumulants) - 1):
            n = itrc + 1
            integrand_left[itrt] += (
                (-1) ** n
                / factorial(n)
                * (betas[itrt + 1] - betas[itrt]) ** n
                * (betas[itrt] * cumulants[itrc + 1][itrt] - (n - 1) * cumulants[itrc][itrt])
            )

    for itrt in range(1, n_chain):
        for itrc in range(len(cumulants) - 1):
            n = itrc + 1
            integrand_right[itrt - 1] -= (
                (-1) ** n
                / factorial(n)
                * (betas[itrt - 1] - betas[itrt]) ** n
                * (betas[itrt] * cumulants[itrc + 1][itrt] - (n - 1) * cumulants[itrc][itrt])
    ...
    integrand_avg = (integrand_left + integrand_right) / 2.0
    ...
    for itrt1 in range(betas_new.size):
        ...
        for itrc in range(1, len(cumulants) - 1):
            ...
            estim_left[itrt1] += (
                ...
                / factorial(n)
                * (
                    (n**2 - n) * cumulants[itrc - 1, itrt_old]
                    + 2 * n * beta0 * cumulants[itrc, itrt_old]
                    + beta0**2 * cumulants[itrc + 1, itrt_old]
                )
            )
        ...
        if itrt_old < n_chain - 1:
            ...
            for itrc in range(1, len(cumulants) - 1):
                ...
                estim_right[itrt1] += (
                    ...
                    / factorial(n)
                    * (
                        (n**2 - n) * cumulants[itrc - 1, itrt_old + 1]
                        + 2 * n * beta0 * cumulants[itrc, itrt_old + 1]
                        + beta0**2 * cumulants[itrc + 1, itrt_old + 1]
                    )
                )
            ...
            if itrt_old == 0:
                ...
                estim_right[itrt1] = np.exp(-(((1.0 / beta0 - 1.0 / beta1) * beta0) ** 2) / 10) * estim_right[itrt1]

            if estim_left[itrt1] != 0.0:
                dbeta = betas_orig[itrt_old + 1] - betas_orig[itrt_old]
                estim_center[itrt1] = (betas_orig[itrt_old + 1] - beta1) / dbeta * estim_left[itrt1] + (
                    beta1 - betas_orig[itrt_old]
                ) / dbeta * estim_right[itrt1]
            else:
                estim_center[itrt1] = estim_right[itrt1]
        else:
            estim_center[itrt1] = estim_left[itrt1]

    estim_left[estim_left < 0.0] = 0.0
    estim_right[estim_right < 0.0] = 0.0
    estim_center[estim_center < 0.0] = 0.0
